...
const cleanTwitterURL = (url) => {
  const regex = /(https:\/\/twitter\.com\/[^\/]+\/status\/\d+)(\/photo\/\d+)?(\/\d+)?/i;
...
const getTwitterMedia = async (url, options = {}) => {
  try {
    const input = typeof url === 'object' ? (url.url ? url : { found: false, error: 'No URL provided' }) : { url };
    ...
    const result = await axios.get(apiURL).then((res) => res.data).catch(() => ({ found: false, error: 'An issue occurred. Make sure the Twitter link is valid.' }));
