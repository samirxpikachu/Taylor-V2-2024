...
async function getRandomUserAgent() {
  return { 'User-Agent': FakeUserAgent() };
}

async function getInfo() {
  try {
    const response = await fetch("https://snaptwitter.com", { headers: await getRandomUserAgent() });
    const $ = cheerio.load(await response.text());
    const formInputs = $('form#get_video input[name]');
...
async function scrapeTwitterMedia(url) {
  ...
  try {
    ...
    const $ = cheerio.load((await response.json()).data.toString());
