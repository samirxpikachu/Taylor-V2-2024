        ...
        sendFile: {
            ...
            let text = caption;
                ...
                */
                ...
                let message = {
                    ...Object.assign(
  { mentions: await conn.parseMention(text), contextInfo: { mentionedJid: await conn.parseMention(text) } },
  { 
    ...options || {},
    ...(conn.temareply?.contextInfo && { contextInfo: { ...options?.contextInfo || {}, ...conn.temareply.contextInfo, externalAdReply: { ...options?.contextInfo?.externalAdReply || {}, ...conn.temareply.contextInfo.externalAdReply } } }) 
  }
)
,
                    caption: text,
...
,
        ...
        reply: {
            ...
            async value(jid, text = '', quoted, options) {
                return Buffer.isBuffer(text) ? await conn.sendFile(jid, text, 'file', '', quoted, false, ...Object.assign(
  { mentions: await conn.parseMention(text), contextInfo: { mentionedJid: await conn.parseMention(text) } },
  { 
    ...options || {},
    ...(conn.temareply?.contextInfo && { contextInfo: { ...options?.contextInfo || {}, ...conn.temareply.contextInfo, externalAdReply: { ...options?.contextInfo?.externalAdReply || {}, ...conn.temareply.contextInfo.externalAdReply } } }) 
  ...
  { 
    ...
    ...(conn.temareply?.contextInfo && { contextInfo: { ...options?.contextInfo || {}, ...conn.temareply.contextInfo, externalAdReply: { ...options?.contextInfo?.externalAdReply || {}, ...conn.temareply.contextInfo.externalAdReply } } }) 
        ...
        parseMention: {
             ...
             */
            async value(text = '') {
                const result = [...text.matchAll(/@([^\s]+)(?=\s|$)/g)].map(match => match[1] ? `${match[1].replace(/[^\d]/g, '').split(/[ \n]/)[0]}@s.whatsapp.net` : null).filter(entry => entry && /\d/.test(entry));
                return result;
            },
            enumerable: true
        },
        ...
        sendStimg: {
            async value(jid, path, quoted, options = {}) {
                let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,` [1], 'base64') : /^https?:\/\//.test(path) ? await (await fetch(path)).arrayBuffer() : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
                let buffer
