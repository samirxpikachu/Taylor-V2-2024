...
class Gemini {
  ...
  async generateGeminiProVisionContent(text, data, mime) {
    return await this.request(
      ...
      'POST',
      { contents: [{ parts: [{ text }, { inline_data: { mime_type: mime, data } }] }] },
      { key: this.apiKey }
    );
  }
  ...
  async geminiProVision() {
    return await this.request('gemini-pro-vision', 'GET', null, { key: this.apiKey });
  }
  ...
  async request(endpoint, method, data, params) {
    try {
      ...
      const headers = method === 'POST' ? { 'Content-Type': 'application/json' } : { key: this.apiKey };
