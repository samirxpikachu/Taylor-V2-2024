...
class VyroAI {
    constructor({ key = "vk-dk3DMbasbyjxOeOVu9k8lqnDAiKYs1NaqMEOBT6kwzHJS", link = true, buffer = true }) {
        this.baseUrl = "https://api.vyro.ai/v1/imagine/api/";
        this.commonHeaders = {
            Authorization: `Bearer ${key}`,
        };
        this.defaultOptions = { url: link, buffer: buffer };
    }
    ...
    async makeRequest(url, formdata, options = {}) {
        ...
        try {
            ...
            if (mergedOptions.url && !mergedOptions.buffer) {
                return { link: await this.catbox(await response.arrayBuffer()) };
            } else if (!mergedOptions.url && mergedOptions.buffer) {
                return { buffer: Buffer.from(await response.arrayBuffer()) };
            } else if (mergedOptions.url && mergedOptions.buffer) {
                return { buffer: Buffer.from(await response.arrayBuffer()), link: await this.catbox(await response.arrayBuffer()) };
            } else {
                return;
            }
        } catch (error) {
    ...
    async remixImage(prompt, styleId, imageBuffer) {
        ...
        const image = new Blob([imageBuffer.toArrayBuffer()], { type: mime });
        const formdata = new FormData();
        formdata.append("style_id", styleId);
        formdata.append("prompt", prompt);
    ...
    async inpaintImage(prompt, imageBuffer, maskBuffer) {
        ...
        const image = new Blob([imageBuffer.toArrayBuffer()], { type: img.mime });
        const msk = await fileTypeFromBuffer(maskBuffer) || {};
        const mask = new Blob([maskBuffer.toArrayBuffer()], { type: msk.mime });
        const formdata = new FormData();
        formdata.append("prompt", prompt);
        formdata.append("image", image);
