    ...
    try {
        ...
        if (type === "name") {
            const usernamesResponse = await fetch("https://users.roblox.com/v1/usernames/users", {
                ...
                body: JSON.stringify({ usernames: [margs.join(" ")] }),
            });
        ...
        const robloxBadgesResponse = await fetch("https://accountinformation.roblox.com/v1/users/" + id + "/roblox-badges");
        ...
        const usernameHistoryResponse = await fetch("https://users.roblox.com/v1/users/" + id + "/username-history?limit=100&sortOrder=Desc");
        ...
        const thumbnailResponse = await fetch("https://thumbnails.roblox.com/v1/users/avatar?userIds=" + id + "&size=150x150&format=Png&isCircular=false");
        const thumbnailData = await thumbnailResponse.json();
        const premiumStatusResponse = await fetch("https://premiumfeatures.roblox.com/v1/users/" + id + "/validate-membership");
